...
def _bridge_datasource_details(data_source_configs) -> int:
    ...
    for ds_config in data_source_configs:
        ...
        if source_type == "TUSHARE":
            ...
            if not endpoint and ds_config.config_params:
                endpoint = ds_config.config_params.get("api_url")
            if endpoint:
                os.environ["TUSHARE_API_URL"] = endpoint
                logger.debug(f"  ✓ 桥接 TUSHARE_API_URL: {endpoint}")
                bridged_count += 1
