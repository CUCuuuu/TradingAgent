        ...
        try:
            ...
            if ds_type == "tushare":
                ...
                try:
                    ...
                    from tradingagents.dataflows.providers.china.tushare_client import (
                        create_tushare_api,
                    )
                    api_url = ds_config.endpoint
                    if not api_url and ds_config.config_params:
                        api_url = ds_config.config_params.get("api_url")
                    pro = create_tushare_api(api_key, api_url or os.getenv("TUSHARE_API_URL"))
                    # 获取交易日历（轻量级测试）
                    ...
                    if df is not None and len(df) > 0:
                        ...
                        return {
                            ...
                            "details": {
                                ...
                                "api_url": api_url or os.getenv("TUSHARE_API_URL") or "http://api.tushare.pro",
