...
from .tushare_client import create_tushare_api, normalize_tushare_api_url
...
class TushareProvider(BaseStockDataProvider):
    ...
    def __init__(self):
        ...
        self.api_url = normalize_tushare_api_url(self.config.get('api_url'))
    ...
    def _get_api_url_from_database(self) -> Optional[str]:
        """Read a custom endpoint from the active Tushare data source config."""
        try:
            from app.core.database import get_mongo_db_sync
            ...
            for ds_config in (config_data or {}).get('data_source_configs', []):
                if ds_config.get('type') != 'tushare':
                    continue
                endpoint = ds_config.get('endpoint')
                if not endpoint:
                    endpoint = (ds_config.get('config_params') or {}).get('api_url')
                if endpoint:
                    return endpoint
        except Exception as e:
            self.logger.debug(f"读取 Tushare API 地址失败，使用环境变量: {e}")
        return None

    def _create_api(self, token: str):
        """Create a client using the database endpoint or environment endpoint."""
        database_url = self._get_api_url_from_database()
        self.api_url = normalize_tushare_api_url(
            database_url or self.config.get('api_url') or os.getenv('TUSHARE_API_URL')
        )
        self.logger.info(f"🔌 Tushare API 地址: {self.api_url}")
        return create_tushare_api(token, self.api_url)
        ...
        try:
            ...
            if db_token:
                try:
                    ...
                    self.api = self._create_api(db_token)
            ...
            if env_token:
                try:
                    ...
                    self.api = self._create_api(env_token)
